...
async function testEmbeddingRetrievalAPI() {
  ...
  try {
    ...
    const servicePath = path.join(__dirname, '../src/main/services/EmbeddingRetrievalService.ts');
    ...
    if (fs.existsSync(servicePath)) {
      ...
      logTest('EmbeddingRetrievalService.searchSimilar() method exists', hasSearchSimilar);
      ...
      const hasSearchQuery = serviceContent.includes('export interface SearchQuery');
      const hasSearchResponse = serviceContent.includes('export interface SearchResponse');
      const hasSimilarityResult = serviceContent.includes('export interface SimilarityResult');
      ...
      logTest('EmbeddingRetrievalService file exists', false, 'Service file not found');
    }
  ...
  try {
    ...
    if (fs.existsSync(servicePath)) {
      ...
      const hasPerformanceWarning = serviceContent.includes('200') && serviceContent.includes('warn');
      const hasOptimizedCalculation = serviceContent.includes('vectorized') || serviceContent.includes('optimized');
      const hasPerformanceConstants = serviceContent.includes('DEFAULT_TOP_N') || serviceContent.includes('MAX_TOP_N');
  ...
  try {
    ...
    if (fs.existsSync(servicePath)) {
      ...
      const hasCacheTTL = serviceContent.includes('5 * 60 * 1000') || serviceContent.includes('CACHE_TTL');
      const hasCacheMap = serviceContent.includes('Map') && serviceContent.includes('cache');
      const hasCacheCleanup = serviceContent.includes('cleanup') || serviceContent.includes('evict');
  ...
  try {
    const docsPath = path.join(__dirname, '../docs/embedding-retrieval-api.yaml');
    const fs = await import('fs');
    ...
    if (fs.existsSync(docsPath)) {
      ...
      const hasSimilaritySearchEndpoint = docContent.includes('/similarity-search');
      ...
      logTest('similarity-search endpoint documented', hasSimilaritySearchEndpoint);
      ...
      logTest('OpenAPI documentation file exists', false, 'docs/embedding-retrieval-api.yaml not found');
    }
  ...
  try {
    ...
    if (fs.existsSync(mainPath)) {
      ...
      const hasIPCHandler = mainContent.includes('similarity-search') && mainContent.includes('handleSimilaritySearch');
  ...
  try {
    ...
    if (fs.existsSync(servicePath)) {
      ...
      const hasRepositoryPattern = serviceContent.includes('EmbeddingRepo') && 
                                  serviceContent.includes('EvidenceRepo') && 
                                  serviceContent.includes('PersonaRepo');
      const hasLangGraphIntegration = serviceContent.includes('LangGraphService');
      ...
      const hasTypeScript = serviceContent.includes('interface') && serviceContent.includes('Promise<');
  ...
  try {
    ...
    if (fs.existsSync(servicePath)) {
      ...
      const hasCosineSimilarityMath = serviceContent.includes('dotProduct') && 
                                     serviceContent.includes('normA') && 
                                     serviceContent.includes('normB');
      const hasVectorValidation = serviceContent.includes('length') && serviceContent.includes('match');
      const hasZeroVectorHandling = serviceContent.includes('normA === 0') || serviceContent.includes('normB === 0');
      const hasSimilarityThreshold = serviceContent.includes('minSimilarity');
      const hasTopNLimiting = serviceContent.includes('topN') && serviceContent.includes('slice');
