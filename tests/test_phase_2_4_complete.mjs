...
async function testRepositoryPagination() {
  ...
  try {
    ...
    const { PersonaRepo } = await import('../dist/main/main/db/repositories/PersonaRepo.js');
    const { EmbeddingRepo } = await import('../dist/main/main/db/repositories/EmbeddingRepo.js');
    ...
    logTest('PersonaRepo pagination interface', hasValidPagination, 
    ...
    logTest('PersonaRepo advanced filtering', hasValidFiltering,
    ...
    logTest('EmbeddingRepo pagination interface', hasValidEmbeddingPagination,
    ...
    logTest('EmbeddingRepo statistics', hasValidStats,
...
async function testEncryptionSecurity() {
  ...
  try {
    ...
    logTest('CRITICAL: Row-level encryption security', encryptionTestsPassed,
      encryptionTestsPassed ? 'All encryption tests passed' : 'Some encryption tests failed');
...
async function testCLISeeding() {
  ...
  try {
    ...
    logTest('CLI help command', helpWorking, 
    ...
    const statsWorking = statsExitCode === 0 && statsOutput.includes('Database Statistics');
    logTest('CLI stats command', statsWorking,
    ...
    if (statsOutput.includes('Personas: 0')) {
      ...
      const seedProcess = spawn('node', ['scripts/db-seed-mock.js', 'seed', 'small'], {
        stdio: 'pipe'
      });
      ...
      const seedWorking = seedExitCode === 0 && seedOutput.includes('seeding completed successfully');
      logTest('CLI small seeding', seedWorking,
        seedWorking ? 'Small dataset seeded successfully' : 'Seeding failed');
    } else {
      logTest('CLI seeding (skipped)', true, 'Database already has data - skipping seeding test');
    }
...
async function testERDiagram() {
  ...
  try {
    ...
    const erDiagramPath = path.join(__dirname, '..', 'docs', 'database_er_diagram.md');
    ...
    if (erDiagramExists) {
      ...
      logTest('ER diagram content completeness', hasValidContent,
        hasValidContent ? 'All required sections present' : 'Missing required sections');
      ...
      logTest('Mermaid diagram syntax', mermaidSyntaxValid,
        mermaidSyntaxValid ? 'Valid Mermaid ER syntax' : 'Invalid Mermaid syntax');
    }
...
async function testIntegrationFeatures() {
  ...
  try {
    ...
    logTest('Repository enhanced methods', personaMethodsExist && embeddingMethodsExist,
    ...
    try {
      ...
      logTest('Repository interfaces exported', false, 'Interfaces not properly exported');
    }
    ...
    logTest('Functional pagination', paginationWorking,
